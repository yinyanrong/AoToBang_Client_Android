...
import com.handmark.pulltofresh.library.custom.TweenAnimLoadingLayout;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	private void pullEvent() {
		...
		if (newScrollValue != 0 && !isRefreshing()) {
			...
			switch (mCurrentMode) {
				case PULL_FROM_END:
					mFooterLayout.onPull(scale,Math.abs(newScrollValue));
					break;
				case PULL_FROM_START:
				default:
					mHeaderLayout.onPull(scale,Math.abs(newScrollValue));
					break;
			}

			if (mState != State.PULL_TO_REFRESH && itemDimension >= Math.abs(newScrollValue)) {
				setState(State.PULL_TO_REFRESH);
				mHeaderLayout.isReleaseToRefresh=false;
			} else if (mState == State.PULL_TO_REFRESH && itemDimension+mHeaderLayout.getExtraHeigh() +mHeaderLayout.getLayoutPanding()< Math.abs(newScrollValue)) {
				setState(State.RELEASE_TO_REFRESH);
				mHeaderLayout.isReleaseToRefresh=true;
			}
		}
	}
