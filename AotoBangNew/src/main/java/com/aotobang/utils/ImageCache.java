...
public class ImageCache {
	private static final int MAXSIZE = 1024 * 1024 * 10;// 5M
	...
	private ImageCache() {
		...
		lrucache = new LruCache<Object, Bitmap>(MAXSIZE) {
			...
			protected int sizeOf(Object key, Bitmap value) {
				// 每添加一张图片，size的变动代表该bitmap占用的内存大小
				 Log.i(TAG, key.toString()+"--value"+getSize(value));
		...
		if (Environment.getExternalStorageState().equals(
			...
			String path = AotoBangApp.getInstance().getCachePath(PathType.Img);
			LogUtil.info(ImageCache.class, path);
			diskLruCache = DiskLruCache.openCache(AotoBangApp.mPreLoad.appContext, new File(
					path), DIS_CACHE_SIZE);
		}
	...
	public void put(Object key, Bitmap value,boolean diskCache) {
		// 如果一旦添加的Bitmap所占据的总的内存超过了MAXSIZE，移除一部分不常使用的map
		lrucache.put(key, value);

		if (diskLruCache != null&&diskCache) {
			diskLruCache.put(key.toString(), value);
		}
	}
	public Bitmap get(Object key) {
		...
		LogUtil.info(ImageCache.class, bitmap+"key--"+key);
		// 如果没有
		if (bitmap == null) {
			// 读取SDCard中得资源
			if (diskLruCache != null) {
