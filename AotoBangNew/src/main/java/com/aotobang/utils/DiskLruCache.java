...
import com.aotobang.app.BuildConfig;
...
public class DiskLruCache {
    ...
    private final  Map<String, String> mLinkedHashMap = Collections.synchronizedMap(new LinkedHashMap<String, String>(INITIAL_CAPACITY,
            LOAD_FACTOR, true));
    private final  Map<String, Bitmap> bitmaps = Collections.synchronizedMap(new LinkedHashMap<String, Bitmap>(INITIAL_CAPACITY,
            LOAD_FACTOR, true));
          ...
          //  final String file = mLinkedHashMap.get(key);
           /* if (file != null) {
                ...
                if (new File(existingFile).exists()) {
                	...
                	Bitmap bit=BitmapFactory.decodeFile(existingFile);
                	if(bit!=null)
                		bitmaps.put(key, bit);
                    LogUtil.info(DiskLruCache.class, key);
                    if (BuildConfig.DEBUG) {
    ...
    public boolean containsKey(String key) {
        ...
        if (mLinkedHashMap.containsKey(key)&&bitmaps.containsKey(key)) {
            return true;
        }
    ...
    //    try {
           ...
           // return cacheDir.getAbsolutePath() + File.separator + CACHE_FILENAME_PREFIX + URLEncoder.encode(key.replace("*", ""), "UTF-8");
            return cacheDir.getAbsolutePath() + File.separator + CACHE_FILENAME_PREFIX + PathUtil.getFileName(key);
